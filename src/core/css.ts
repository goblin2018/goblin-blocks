...
const Css = {
  ...
  Content: [
    ...
    */
    ...
    'border: 1px solid;',
    'border-radius: 4px;',
    'box-shadow: 0px 0px 8px 1px ' + Colors.dropDownShadow + ';',
    'padding: 4px;',
    '-webkit-user-select: none;',
    'min-height: 15px',
    '}',
    ...
    'box-shadow: 0px 0px 0px 4px ' + Colors.fieldShadow + ';',
    '}',

    '.blocklyDropDownButton:active {',
    'box-shadow: 0px 0px 0px 6px ' + Colors.fieldShadow + ';',
    '}',
    ...
    */
    ...
    '.blocklyAngleCircle {',
    'stroke: ' + Colors.motion.tertiary + ';',
    'stroke-width: 1;',
    'fill: ' + Colors.motion.secondary + ';',
    '}',
  ...
  inject: (hasCss: boolean, pathToMedia: string) => {
    if (Css.styleSheet_) {
      return
    }
    ...
    Css.mediaPath_ = pathToMedia.replace(/[\\\/]$/, '')
    text = text.replace(/<<<PATH>>>/g, Css.mediaPath_)
    // Dynamically replace colours in the CSS text, in case they have
    // been set at run-time injection.
    for (let colourProperty in Colors) {
      let c = Colors.getColor(colourProperty)
      if (c) {
        // Replace all
        text = text.replace(new RegExp('\\$colour\\_' + colourProperty, 'g'), c)
      }
    }
